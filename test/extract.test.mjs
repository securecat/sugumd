...
check("print-hidden block removed", !md.includes("印刷する"));
check("mail share link removed", !md.includes("mailto:?"));
check("SNS share links removed", !md.includes("シェアする"));
check("no empty list skeletons left", !/^-\s*$/m.test(md));
check("contact mailto link kept", md.includes("mailto:info@example.com"));
check("button removed but paragraph kept", !md.includes("もっと見る") && md.includes("さらに本文が続きます"));
