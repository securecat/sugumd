...
- **Frame-aware** — on pages where the content lives in a dominant same-origin frame (e.g. web archive viewers like megalodon.jp), the frame's content is extracted instead of the wrapper
...
- **フレーム対応** — コンテンツが大きな同一オリジンフレームの中にあるページ（megalodon.jp のようなウェブ魚拓ビューア等）では、外側ではなくフレームの中身を抽出
